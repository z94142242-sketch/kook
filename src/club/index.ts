...
export function startClubSystem(): ClubRuntime {
  getDb();
  console.log(`[club] db ready at ${config.club.dbPath}`);

  let httpServer: ServerType | undefined;
  if (config.club.http.enabled) {
    httpServer = serve({
      fetch: buildHttpApp().fetch,
      hostname: config.club.http.host,
      port: config.club.http.port
    });
    console.log(`[club] http api listening on ${config.club.http.host}:${config.club.http.port}`);
  }
