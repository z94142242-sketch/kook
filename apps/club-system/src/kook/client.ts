...
export class KookClient {
  ...
  /**
   * 把指定用户搬运到目标语音频道。
   * 官方接口：POST /api/v3/channel/move-user
   * 要求：用户当前已经在某个语音频道里，且机器人在目标频道有「搬运语音用户」权限。
   */
...
function mapEvent(raw: RawKookEvent): KookSystemEvent | null {
  ...
  // KOOK 系统事件：语音进出。type=255，extra.type 是字符串
  ...
  // 文本 / 卡片消息：type=1 (text), 9 (kmarkdown), 10 (card)
